    ...
    public function performAction()
    {
        ...
        switch ($action) {
                ...
                try {
                    ...
                    if (isset($body['app']['user']) && $body['app']['user']['authenticated']) {
                        if (isset($body['app']['token'])) {
                            ...
                            $sites = $api->sites($body['app']['user']['lastUsedWorkspace']);
                            $sites = json_decode($sites['body'], true);
            ...
            case 'loadWorkspaceTokens':
                ...
                $sites = $api->sites(sanitize_text_field($_POST['workspace']));

                if (empty($tokens['body']) || empty($sites['body'])) {
                  return;
                }
            ...
            case 'updateToken':
                ...
                foreach ($options['sites'] as $site) {
                    if ($site['apiToken']['tokenId'] == $tokenId) {
                        $options['site'] = $site;
                    }
                }
